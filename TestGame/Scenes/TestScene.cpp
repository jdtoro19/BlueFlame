...
TestScene::TestScene()
{
	...
	model->physicsComponent->setPhysicsType(PhysicsComponent::Physics_Type::STATIC);
	model->physicsComponent->SetMass(500.0f);
...
void TestScene::Update(const float deltaTime)
{
	...
	if (Physics::isColliding(cube->collisionComponent, floor->collisionComponent)) {
		Physics::Collide(cube->physicsComponent, cube->collisionComponent, floor->physicsComponent, floor->collisionComponent, 1.0f);
	...
	if (Physics::isColliding(cube2->collisionComponent, floor->collisionComponent)) {
		Physics::Collide(cube2->physicsComponent, cube2->collisionComponent, floor->physicsComponent, floor->collisionComponent, 0.4f);
	}
	else {
	...
	if (Physics::isColliding(cube->collisionComponent, cube2->collisionComponent)) {
		Physics::Collide(cube->physicsComponent, cube->collisionComponent, cube2->physicsComponent, cube2->collisionComponent, 0.2f);
	}
	else {
	...
	if (Physics::isColliding(cube->collisionComponent, model->collisionComponent)) {
		Physics::Collide(cube->physicsComponent, cube->collisionComponent, model->physicsComponent, model->collisionComponent, 0.2f);
	}

	if (Physics::isColliding(model->collisionComponent, floor->collisionComponent)) {
		Physics::Collide(model->physicsComponent, model->collisionComponent, floor->physicsComponent, floor->collisionComponent, 0.5f);
	}

	if (Physics::isColliding(model->collisionComponent, cube2->collisionComponent)) {
		Physics::Collide(model->physicsComponent, model->collisionComponent, cube2->physicsComponent, cube2->collisionComponent, 0.5f);
	}
}
