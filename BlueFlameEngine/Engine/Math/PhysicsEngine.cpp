...
bool PhysicsEngine::isColliding(CollisionComponent* a, CollisionComponent* b) {
	...
	if (a->GetCollisionType() == CollisionComponent::Collision_Type::SPHERE &&
		...
		// Calculates squared distance between centres
		glm::vec3 d = a->GetBoundingSphere().c + b->GetBoundingSphere().c;
		float d2 = glm::dot(d, d);

		// If the squared distance is less than the squared sum of radii, spheres intersect
		float radiusSum = a->GetBoundingSphere().r + b->GetBoundingSphere().r;
		if (d2 <= radiusSum * radiusSum) {
			return true;
		}
		else {
		...
		// Find point p on AABB closest to the centre of the sphere
		glm::vec3 p;
		ClosestPointOnAABB(a->GetBoundingSphere().c, b->GetBoundingBox(), p);

		// If the squared distance from sphere centre to point p is less than the squared sphere radius, box and sphere intersect
		glm::vec3 v = p - a->GetBoundingSphere().c;
		if (glm::dot(v, v) <= a->GetBoundingSphere().r * a->GetBoundingSphere().r) {
			return true;
		}
		else {
			return false;
		...
		// If the squared distance from sphere centre to point p is less than the squared sphere radius, box and sphere intersect
		glm::vec3 v = p - b->GetBoundingSphere().c;
		if (glm::dot(v, v) <= b->GetBoundingSphere().r * b->GetBoundingSphere().r) {
			return true;
		}
		else {
			return false;
		...
		// If no intersection along an axis, boxes are not colliding
		if (glm::abs(a->GetBoundingBox().c.x - b->GetBoundingBox().c.x) > (a->GetBoundingBox().r.x + b->GetBoundingBox().r.x)) return false;
		if (glm::abs(a->GetBoundingBox().c.y - b->GetBoundingBox().c.y) > (a->GetBoundingBox().r.y + b->GetBoundingBox().r.y)) return false;
		if (glm::abs(a->GetBoundingBox().c.z - b->GetBoundingBox().c.z) > (a->GetBoundingBox().r.z + b->GetBoundingBox().r.z)) return false;
